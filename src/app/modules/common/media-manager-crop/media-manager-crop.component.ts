import { Component, OnInit, ViewChild, ElementRef,  ViewChildren, QueryList, Input,  Output, EventEmitter } from '@angular/core';
...
export class MediaManagerCropComponent implements OnInit {
 ...
 dataURItoBlob(dataURI:any) {
  ...
  async onEdit(){
    ...
    await this.toDataURL(this.imageUrl, function (dataUrl:any) { src=dataUrl; })
  ...
  async imageCropped(event:any) {
    ...
    console.log("🚀 ~ file: media-manager-crop.component.ts:294 ~ MediaManagerCropComponent ~ imageCropped ~ event.base64", event.base64)
...
// Required
  ...
  imageLoaded(event:any) {
    ...
    this.showCropper = true;
      ...
      console.log("🚀 ~ file: media-manager-crop.component.ts:374 ~ MediaManagerCropComponent ~ imageLoaded ~  this.previewWidth",  this.previewWidth)
      this.calculateRatio( this.previewWidth,this.previewHeight);
...
// Required
  ...
  var file = new File([this.imageBase64String], this.imageNmae, { type: this.type });
    var mediaUploaded = await this.uploadSingleMediaToFireBaseAndGetUrl(file, this.imageNmae);
