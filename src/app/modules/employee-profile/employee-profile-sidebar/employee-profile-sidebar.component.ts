...
export class EmployeeProfileSidebarComponent implements OnInit {
  ...
  searchPosition(event: Event): void {
    ...
    this.filteredPositions = constant.jobTitles.filter(position =>
      position.toLowerCase().includes(inputValue.toLowerCase())
    );
  }
  changeSalary:boolean=false;
  updateSalaryToggle(){
    ...
    this.myForm.get('newCTC')?.updateValueAndValidity();
  }
  ...
  onSubmit() {
    ...
    if (this.myForm.valid) {
      const userPositionDTO: UserPositionDTO = {
        ...
        annualCTC: this.myForm.get('newCTC')?.value
      };
      this.isSaving = true;
