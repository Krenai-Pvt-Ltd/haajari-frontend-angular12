...
export class AttendanceLeaveComponent implements OnInit {
  ...
  loadLeaveLogsOld(): void {
    const leaveType =
      this.selectedLeaveType === 'All' ? undefined : this.selectedLeaveType;
    const status =
  ...
  saveLeaveRequestUser() {
    ...
    this.userLeaveRequest.leaveType =
      this.userLeaveRequest.userLeaveTemplateId.toString();
    this.dataService
  ...
  getUserLeaveReq() {
    ...
    this.dataService
      .getUserLeaveRequests(this.userId, 0, 0)
      .subscribe((res: any) => {
  ...
  totalAttendanceDetails: TotalEmployeeProfileAttendanceResponse =
    new TotalEmployeeProfileAttendanceResponse();
    ...
    this.dataService
      ...
      .subscribe(
        (response) => {
          ...
          this.attendanceDetails =
            response.object.employeeProfileAttendanceResponseList;
          this.totalAttendanceDetails =
            response.object.totalEmployeeProfileAttendanceResponse;
        },
        (error) => {
          this.isShimmer = false;
          console.log(error);
  ...
  convertToHourMinuteFormat(time: string | null | undefined): string {
    ...
    const [hours, minutes, seconds] = timeWithoutSign
      .split(':')
    ...
    let result = '';
    if (hours !== 0) {
      result += `${isNegative ? '-' : ''}${Math.abs(hours)} hr${
        Math.abs(hours) !== 1 ? 's' : ''
      }`;
    }
    if (minutes !== 0 || result === '') {
      // Include minutes even if they are zero when there's no hour part
      result += `${result ? ', ' : isNegative ? '-' : ''}${Math.abs(
        minutes
      )} min${Math.abs(minutes) !== 1 ? 's' : ''}`;
    }

    return result;
  }

  convertToHourMinuteFormatNegativeCase(
  ...
  disableMonths = (date: Date): boolean => {
    ...
    const userRegistrationYear = new Date(this.userJoiningDate).getFullYear();
    const userRegistrationMonth = new Date(this.userJoiningDate).getMonth();

    // Disable if the month is before the organization registration month
    if (
      dateYear < userRegistrationYear ||
      (dateYear === userRegistrationYear && dateMonth < userRegistrationMonth)
    ) {
      return true;
    }
  ...
  @ViewChild('timeline', { static: true }) timelineContainer:
    | ElementRef
    | undefined;
            ...
            tooltip: {
              callbacks: {
                label: (tooltipItem: any) => {
                  const formattedTime = this.formatDecimalToTime(
                    tooltipItem.raw
                  );
  ...
  onRequestChange(value: string) {
    if (value === 'Attendance update') {
      ...
      const attendanceUpdateButton =
        document.getElementById('attendanceUpdate');
  ...
  getAttendanceChecktimeListDate(statusString: string): void {
    ...
    this.dataService
      .getAttendanceChecktimeList(this.userId, formattedDate, statusString)
      .subscribe(
        (response) => {
  ...
  resetError() {
    if (
      this.attendanceTimeUpdateForm.get('attendanceRequestType')?.value ===
      'UPDATE'
    ) {
  ...
  @ViewChild('closeAttendanceUpdateModal')
  closeAttendanceUpdateModal!: ElementRef;
  ...
  submitForm(): void {
    ...
    attendanceTimeUpdateRequest.requestType = this.attendanceTimeUpdateForm.get(
      'attendanceRequestType'
    )?.value;
    attendanceTimeUpdateRequest.choosenDateString = this.choosenDateString;
    this.dataService
      .sendAttendanceTimeUpdateRequest(attendanceTimeUpdateRequest)
      .subscribe(
        (response) => {
          ...
          if (response.status === true) {
            ...
            this.attendanceTimeUpdateForm
              .get('attendanceRequestType')
              ?.setValue('UPDATE');
  ...
  onDateChange(date: Date | null): void {
    if (
      ...
      this.statusString =
        this.attendanceTimeUpdateForm.get('updateGroup.requestType')?.value ||
        '';
      ...
      this.statusString =
        this.attendanceTimeUpdateForm.get('createGroup.requestType')?.value ||
        '';
      this.getHolidayForOrganization(this.selectedDateAttendance);
      this.getAttendanceExistanceStatus(this.selectedDateAttendance);
    }
  ...
  getHolidayForOrganization(selectedDate: any) {
    ...
    this.dataService
      .getHolidayForOrganization(
        this.helperService.formatDateToYYYYMMDD(selectedDate)
      )
      .subscribe(
        (response) => {
  ...
  isAttendanceFormValid(): boolean {
    ...
    if (
      ...
      return (
        (updateGroup ? updateGroup.valid : false) &&
      ...
      return (
        (createGroup ? createGroup.valid : false) &&
        (managerId ? managerId.valid : false) &&
        (requestReason ? requestReason.valid : false)
      );
    }
  ...
  onAttendanceRequestTypeChange(): void {
    debugger;
    console.log(
      `Selected Attendance Request Type: ${this.attendanceRequestType}`
    );
  ...
  getAttendanceLogsMethodCall(selectedDate: string) {
    ...
    this.dataService.getAttendanceLogs(this.userId, selectedDate).subscribe(
      (response) => {
        debugger;
  ...
  getUserBreakTimingsReportByDate(date: string) {
    ...
    if (
      ...
      debugger;
      this.dataService
        .getAttendanceDetailsBreakTimingsReportByDateByUser(this.userId, date)
        .subscribe(
          (response) => {
