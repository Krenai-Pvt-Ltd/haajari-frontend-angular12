...
import {
  SalaryTemplate,
  TemplateDeductionResponse,
} from 'src/app/payroll-models/SalaryTemplate';
import { HelperService } from 'src/app/services/helper.service';
import { SalaryTemplateService } from 'src/app/services/salary-template.service';
import { SalaryTemplateSidebarComponent } from '../salary-template-sidebar/salary-template-sidebar.component';
import { UserSalaryTemplate } from 'src/app/payroll-models/UserSalaryTemplate';

@Component({
  selector: 'app-salary-template-copy',
  templateUrl: './salary-template-copy.component.html',
  styleUrls: ['./salary-template-copy.component.css'],
})
export class SalaryTemplateCopyComponent implements OnInit {
  /**
   * Component factor
   */
  ...
  constructor(
    private _salaryTemplateService: SalaryTemplateService,
  ...
  isUserMapping: boolean = false;

  getOrganizationSalaryTemplates() {
    this.shimmer = true;
    this.salaryTemplateList = [];
    if (this.isUserMapping) {
      this.isActive = 1;
      this.shimmer = false;
    }
    this._salaryTemplateService.getSalaryTemplates(this.isActive,this.currentPage,this.itemPerPage,this.search)
      .subscribe(
        (response) => {
          if (response.status) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  mapUser() {
    this.isUserMapping = true;
    this.toggle = true;
    this.getOrganizationSalaryTemplates();
  }

  selectedTemplate(templateId: any) {
    const selectedTemplate = this.salaryTemplateList.find(
      (t) => t.id == templateId
    );
    this.salaryTemplate = JSON.parse(JSON.stringify(selectedTemplate));
  }

  disableMonths = (current: Date): boolean => {
    const now = new Date();
    return current < new Date(now.getFullYear(), now.getMonth(), 1);
  };
  reviseSalary() {
    this.userSalaryTemplate.isRevision = true;
    this.isUserMapping = true;
    this.getOrganizationSalaryTemplates();
  }

  userSalaryTemplate: UserSalaryTemplate = new UserSalaryTemplate();
  saveUserSalaryTemplate() {
  ...
  mapOnTemplate() {
    ...
    if (!this.isNewTemplate) {
      this.templateComponents.earningComponents =
        this.templateComponents.earningComponents.filter((tempComponent) => {
      ...
      this.templateComponents.reimbursementComponents =
        this.templateComponents.reimbursementComponents.filter(
          (tempComponent) => {
            const salaryComponent =
              this.salaryTemplate.reimbursementComponents.find(
      ...
      this.templateComponents.deductions =
        this.templateComponents.deductions.filter((tempComponent) => {
      ...
      const basicComponent = this.templateComponents.earningComponents.find(
        (component) => component.name == 'Basic'
      );
      if (basicComponent) {
        this.salaryTemplate.earningComponents.push({ ...basicComponent });
      }
      const fixedAllowanceComponent =
        this.templateComponents.earningComponents.find(
          (component) => component.name == 'Fixed Allowance'
        );
      if (fixedAllowanceComponent) {
        this.salaryTemplate.earningComponents.push({
          ...fixedAllowanceComponent,
        });
      }
      this.templateComponents.earningComponents =
        this.templateComponents.earningComponents.filter(
          (component) =>
  ...
  toggleEarningComponent(component: EarningComponentTemplate) {
    const salaryIndex = this.salaryTemplate.earningComponents.findIndex(
      (c) => c.name === component.name
    );
    const tempIndex = this.templateComponents.earningComponents.findIndex(
      (c) => c.name === component.name
    );

    if (salaryIndex > -1) {
      const removedComponent = this.salaryTemplate.earningComponents.splice(
        salaryIndex,
        1
      )[0];
      this.templateComponents.earningComponents.push(removedComponent);
    } else if (tempIndex > -1) {
      const freshComponent = this.tempTemplateComponents.earningComponents.find(
        (c) => c.name === component.name
      );
      if (freshComponent) {
  ...
  toggleReimbursementComponent(component: ReimbursementComponent) {
    const salaryIndex = this.salaryTemplate.reimbursementComponents.findIndex(
      (c) => c.type == component.type
    );
    const tempIndex = this.templateComponents.reimbursementComponents.findIndex(
      (c) => c.type == component.type
    );

    if (salaryIndex > -1) {
      const removedComponent =
        this.salaryTemplate.reimbursementComponents.splice(salaryIndex, 1)[0];
      this.templateComponents.reimbursementComponents.push(removedComponent);
    } else if (tempIndex > -1) {
      const freshComponent =
        this.tempTemplateComponents.reimbursementComponents.find(
          (c) => c.type == component.type
        );
      if (freshComponent) {
  ...
  toggleDeductionComponent(component: TemplateDeductionResponse) {
    ...
    const salaryIndex = this.salaryTemplate.deductions.findIndex(
      (comp) => comp.id == component.id
    );
    const tempIndex = this.templateComponents.deductions.findIndex(
      (comp) => comp.id == component.id
    );

    if (salaryIndex > -1) {
      const removedDeduction = this.salaryTemplate.deductions.splice(
        salaryIndex,
        1
      )[0];
      this.templateComponents.deductions.push(removedDeduction);
    } else if (tempIndex > -1) {
      const freshDeduction = this.tempTemplateComponents.deductions.find(
        (comp) => comp.id == component.id
      );
      if (freshDeduction) {
        this.salaryTemplate.deductions.push({ ...freshDeduction });
        this.templateComponents.deductions =
          this.templateComponents.deductions.filter(
            (comp) => comp.id !== component.id
          );
      }
    }
  ...
  findEarning() {
    ...
    this.salaryTemplate.earningComponents.forEach((component) => {
      if (component.valueTypeId == this.VALUE_TYPE_PERCENTAGE) {
        ...
        if (component.calculationBasedId == this.CALCULATION_BASED_CTC) {
          ...
          component.amount =
            (component.value / 100) *
            Math.round(this.salaryTemplate.annualCtc / 12);
        } else if (
          component.calculationBasedId == this.CALCULATION_BASED_BASIC
        ) {
          //basic
        ...
        component.valueTypeId == this.VALUE_TYPE_FLAT &&
        !component.isAdd
      ) {
    ...
    if (count == this.salaryTemplate.earningComponents.length) {
      ...
      if (
        count ==
        this.salaryTemplate.earningComponents.length +
          this.salaryTemplate.reimbursementComponents.length
      ) {
        this.calculatedAmountWithoutFixed =
          this.totalEarningAmount + this.totalReimbursementAmount;
  ...
  mapFinalValue(result: CalculationResult) {
    ...
    var fixedAllowance = this.salaryTemplate.earningComponents.find(
      (x) => x.name === 'Fixed Allowance'
    );
    if (fixedAllowance) {
  ...
  private calculateEPF(fixed: number, hasEPF: boolean): number {
    ...
    if (EPF?.employerContribution == this.EPF_RESTRICTED) {
      ...
      applicableValue =
        epfBase >= this.EPF_RESTRICTED_VALUE
          ? this.EPF_RESTRICTED_VALUE
          : epfBase;
    } else {
  ...
  calculateGrossAmount() {
    ...
    this.salaryTemplate.monthlyCtc = Math.round(
      this.salaryTemplate.annualCtc / 12
    );
    this.salaryTemplate.annualGross = Math.round(
      this.salaryTemplate.monthlyGross * 12
    );
  }
  ...
  saveSalaryTemplate() {
    this.saveLoader = true;
    this.calculateGrossAmount();
    this._salaryTemplateService
      .saveSalaryTemplate(this.salaryTemplate)
      .subscribe(
        (response) => {
          if (response.status) {
            this._helperService.showToast(
              'Your Salary Template saved successfully.',
              Key.TOAST_STATUS_SUCCESS
            );
            this.backFromTemplate();
          } else {
            this._helperService.showToast(
              'Error in saving Salary Template.',
  ...
  toggleStatus(templateId: number) {
    ...
    if (template) {
      template.statusId =
        template.statusId === StatusKeys.ACTIVE
          ? StatusKeys.INACTIVE
          : StatusKeys.ACTIVE;
    }
  }
  ...
  @ViewChild('deleteTemplateCloseButton')
  deleteTemplateCloseButton!: ElementRef;
  ...
  confirmDeleteTemplate() {
    this.saveLoader = true;
    this._salaryTemplateService.deleteTemplate(this.templateId).subscribe(
      (response) => {
        if (response.status) {
          this._helperService.showToast(
